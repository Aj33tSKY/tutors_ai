...
type SessionRow = Pick<Booking, "id" | "subject" | "start_time" | "end_time" | "status" | "lesson_name" | "is_trial" | "stripe_invoice_id" | "amount_gbp_pence" | "started_at" | "recurrence_rule" | "recurrence_series_id"> & { counterpartName: string; counterpartId: string };
...
function SessionDialog({ session, open, onOpenChange, counterpartLabel, canMarkComplete, canSendInvoice }: { session: SessionRow | null; open: boolean; onOpenChange: (open: boolean) => void; counterpartLabel: string; canMarkComplete: boolean; canSendInvoice: boolean }) {
  ...
  const [invoiceState, invoiceAction, invoicePending] = useActionState(sendSessionInvoiceAction, {});
