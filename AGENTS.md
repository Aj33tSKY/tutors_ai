...
- **Adding a `process.env.X` means updating `config/required-env.json`.** CI fails otherwise, in both directions. That manifest is what the production parity check compares both Vercel projects against, so a variable missing from it is a variable nobody verifies exists.
