...
- **Only `develop` and `hotfix/*` may open a pull request into `main`**, enforced by the Promotion source workflow. Dependabot reads its config from the default branch only, so a `target-branch` change takes effect when promoted to `main`, not when merged to `develop`.
